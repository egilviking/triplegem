<?php
/**
* A model for managing TripleGem modules.
*
  ...
  private $TripleGemCMFModules = array('CForm', 'CCPage', 'CCBlog', 'CMUser', 'CCUser', 'CMContent', 'CCContent', 'CFormUserLogin', 'CFormUserProfile', 'CFormUserCreate', 'CFormContent', 'CHTMLPurifier');
    ...
    */
	public function ReadAndAnalyse() {
		$src = TRIPLEGEM_INSTALL_PATH.'/src';
		...
		while (($module = $dir->read()) !== false) {
		  if(is_dir("$src/$module")) {
			if(class_exists($module)) {
			  ...
			  $modules[$module]['isTripleGemCore']   = in_array($rc->name, array('CTripleGem', 'CDatabase', 'CRequest', 'CViewContainer', 'CSession', 'CObject'));
			  $modules[$module]['isTripleGemCMF']    = in_array($rc->name, array('CForm', 'CCPage', 'CCBlog', 'CMUser', 'CCUser', 'CMContent', 'CCContent', 'CFormUserLogin', 'CFormUserProfile', 'CFormUserCreate', 'CFormContent', 'CHTMLPurifier'));
			}
		  }
		}
		$dir->close();
