...
class CMContent extends CObject implements IHasSQL, ArrayAccess {
  ...
  public function Init() {
    try {
      ...
      $this->db->ExecuteQuery(self::SQL('insert content'), array('home', 'page', 'Home page', "This is a demo page, this could be your personal home-page.\n\nLydia is a PHP-based MVC-inspired Content management Framework, watch the making of TripleGemCore at: http://dbwebb.se/TripleGemCore/tutorial.", 'plain', $this->user['id']));
      ...
      $this->db->ExecuteQuery(self::SQL('insert content'), array('download', 'page', 'Download page', "This is a demo page, this could be your personal download-page.\n\nYou can download your own copy of TripleGemCore from https://github.com/mosbth/TripleGemCore.", 'plain', $this->user['id']));
      $this->db->ExecuteQuery(self::SQL('insert content'), array('bbcode', 'page', 'Page with BBCode', "This is a demo page with some BBCode-formatting.\n\n[b]Text in bold[/b] and [i]text in italic[/i] and [url=http://dbwebb.se]a link to dbwebb.se[/url]. You can also include images using bbcode, such as the TripleGemCore logo: [img]http://dbwebb.se/TripleGemCore/current/themes/core/logo_80x80.png[/img]", 'bbcode', $this->user['id']));
